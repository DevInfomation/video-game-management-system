...
@section('content')
    <div class="h-screen flex items-center">
        <form enctype="multipart/form-data" action="{{route('game.submit')}}" method="POST" class="mx-auto w-1/3 border-4 rounded-md border-black bg-primary p-4">
            @csrf
            <h2 class="text-white text-2xl text-center bg-secondary mb-2">Enter the video game</h2>
            <div class="flex flex-col">
                <label class="mb-1 text-white">Add an Image</label>
                <input type="file" name="image" class="border rounded-md p-1.5 mb-2">
